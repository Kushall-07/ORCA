...
Live source: Open-Meteo Weather API. Deterministic normalisation into
``MarineObservation`` records, Redis-cached with bucketed keys, three-tier
fallback (LIVE -> CACHE -> DEMO/MISSING). No LLM.

WMO weather codes 95-99 are preserved verbatim as the ``weather_code``
observation so the deterministic Risk Engine can derive the
**thunderstorm / lightning proxy** itself. This agent never labels that signal
"lightning detection".
"""
...
from app.services.cache import JsonCache, NullCache, weather_cache_key
from app.services.http import HttpClientError

logger = get_logger(__name__)

_KIND = "weather"
_SOURCE_LIVE = "open-meteo-forecast"
_DEMO_FILE = "weather_demo.json"
...
class WeatherAgent:
    ...
    async def fetch(
        ...
        *,
        ...
        try:
            raw = await openmeteo.fetch_weather(
                coordinate.latitude,
                coordinate.longitude,
                url=self.settings.openmeteo_weather_url,
            ...
            payload = normalise_openmeteo(response, when, openmeteo.WEATHER_HOURLY)
            if not payload["values"]:
                raise openmeteo.SchemaValidationError("no usable weather variables in response")
            fetched_at = utcnow()
            ...
            logger.warning("weather live failed", extra={"source": _SOURCE_LIVE})
        ...
        if cached and "fetched_at" in cached and cached.get("values"):
            ...
            if age is not None and age <= self.settings.weather_cache_max_age_seconds:
                stale = age > self.settings.weather_cache_ttl_seconds
                return self._result_from_payload(
                    coordinate,
                    when,
                    cached,
        ...
        return missing_result(
            ...
            coordinate,
            when,
            live_error or "no live, cached or demo weather data available",
        )
    ...
    def _result_from_payload(
        ...
        observations = build_observations(
            ...
            source=status.source if status.tier is DataTier.LIVE else _SOURCE_LIVE,
            source_tier=tier,
        )
        return AgentResult(
            kind=_KIND,
